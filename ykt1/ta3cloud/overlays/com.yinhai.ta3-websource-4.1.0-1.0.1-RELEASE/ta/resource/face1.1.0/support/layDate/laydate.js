...
;!function(){
  ...
  ,laydate = {
    ...
    ,ready: function(fn){
      ...
      ,path = (isLayui ? 'modules/laydate/' : 'css/') + (Base.globvar.FaceSkin||"shebaoBlue") + '.css?v='+ laydate.v + ver;
      ...
      hint: function(content){
        ...
        setValue:function (value) {
            ...
            if(that.config.range){
                ...
                value = value.join(" " + that.config.range + " ")
            }
        ...
        getValue:function () {
            var value = "";
            var elem = that.bindElem || that.config.elem[0];
            if(that.config.range) {
                value = elem.value.length > 0?elem.value.split(' ' + that.config.range + ' '):"";
            }else{
                value = elem.value;
            }
            return value;
        },
        markCalendar:function (mark,calendar) {
            that.remove();
            if(typeof calendar != "undefined")
                that.config.calendar = calendar;
            that.config.mark = lay.extend({}, (that.config.calendar && that.config.lang === 'cn') ? CALENDAR_MARK : {}, mark);
        }
      ,config: that.config
    };
  }
  ...
  */
  ...
  Class.prototype.init = function(){
    ...
    //记录重要日期
    options.mark = lay.extend({}, (options.calendar && options.lang === 'cn') ? CALENDAR_MARK : {}, options.mark);
