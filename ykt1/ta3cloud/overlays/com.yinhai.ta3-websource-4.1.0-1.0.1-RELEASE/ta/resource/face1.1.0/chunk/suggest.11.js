...
(function (factory) {
	...
	function TaSuggest($input, options) {
		/*
   * url 后台请求的路径 Namespace+Action+方法
   * paramIds 作为  Base.submit 的一个参数
   * time 停止输入显示查询结果的延迟时间，默认为500 （ms）
   * digit 输入小于当前位数不进行后台查询，默认为1
   ...
   * height    下拉提示框的高度
   * columns   下拉提示框中显示几列
   * beforeSubmit  提交前的判断，额外参数的传入  return {"dto['inputString']":$input.val(),...} 或者   return true/false
   ...
   * */
			...
			$input.keydown(function (e) {
				...
				if (code != 13) {
					...
					interval = setTimeout(function () {
						...
						var param1 = options.beforeSubmit();
						if (param1) {
							var param = $.extend({ "dto['inputString']": $input.val() }, param1);
							if ($input.val().length >= options.digit) {
								if (!initflag) {
									if (typeof options.width == "undefined") {
										options.width = parseInt($input.css("width"));
									}
									initializeSuggestFramework(options.instance1, options.what, options.dtoname, options.width, options.height, options.columns, options.callBackFun, options.selectColumn, options.oneRowVisible);
									initflag = true;
								}
								sfwQueryUtil_newRPC(options.instance1, options.url, param, options.paramIds);
							}
						}
					}, options.time);
				}
		...
		function sfwCreate(instance, dto) {
			if (sfw[instance].name && sfw[instance].action) {
				...
				sfw[instance].inputContainer.onclick = function () {
					sfwShowOutput(instance);
				};
				...
				if (null == document.getElementById(sfw[instance].name + "_list")) {
					var top = $("#" + dto).position().top;
					var left = $("#" + dto).position().left;
					...
					if (sfw[instance].height == "auto") {
						sfw[instance].outputContainer.style.height = sfw[instance].height;
					} else {
					...
					// 减去border
					sfw[instance].outputContainer.style.left = left - 1 + "px";
					sfw[instance].outputContainer.style.top = top + 32 + "px";
					sfw[instance].outputContainer.style.background = "white";

					sfw[instance].outputContainer.style.zIndex = "55555";
					// 减去padding
					sfw[instance].outputContainer.style.width = sfw[instance].width - 10 + "px";
					sfw[instance].outputContainer.style.minWidth = "150px";
		...
		function sfwHighlight(instance, index) {
			...
			for (var i = 0; i < sfw[instance].suggestions.length; i++) {
				...
				for (var j = 0; j < suggestionColumns.length; j++) {
			...
			for (var k = 0; k < suggestionColumns.length; k++) {
				suggestionColumns[k].className = "SuggestFramework_Highlighted";
			}
		...
		function sfwSelectThis(instance, index) {
			...
			if (sfw[instance].rowcount == 1) {
				...
				if ("function" == typeof sfw[instance].callBackFun) {
					sfw[instance].callBackFun(g_Suggest);
				} else {
      ...
      _sug_tab_row.scrollIntoView(true);
		...
		function sfwShowOutput(instance) {
			var dtoname = sfw[instance].name;
			if (document.getElementById(dtoname).disabled == true || document.getElementById(dtoname).readOnly == true) {} else {
				if (typeof sfw[instance].suggestions != "undefined" && sfw[instance].suggestions.length > 1) sfw[instance].outputContainer.style.display = "block";
			}
		};
		function sfwSuggest(instance, list) {
			...
			var rowcount = 0;
			//=============锁定表头1=========================
			var table1 = "<div id=\"mydiv1_" + instance + "\" class=\"SuggestFramework_div\"  ><table class=\"SuggestFramework_fist_table\" style=\"width:100%;overflow:hidden;\" id=\"" + sfw[instance].name + "_sug_table1\"  cellspacing=\"0\" cellpadding=\"0\" >";
			...
			//=============锁定表头1=========================
			var table = "<div id=\"mydiv_" + instance + "\" class=\"SuggestFramework_div\"  ><table class=\"SuggestFramework_second_table\" style=\"width:100%;overflow:hidden;\" id=\"" + sfw[instance].name + "_sug_table\"  cellspacing=\"0\" cellpadding=\"0\" >";
			var hiddenCol = "";
			if (sfw[instance].heading && sfw[instance].suggestions.length) {
				var heading = sfw[instance].suggestions.shift();
				var thead = "<thead>";
			...
			for (var i = 0; i < sfw[instance].suggestions.length; i++) {
				...
				row.onmouseover = function () {
					sfwHighlight(options.instance1, $(this).index());
				};
				row.onmousedown = function () {
					sfwSelectThis(options.instance1, $(this).index());
				};
			}
			...
			//=============锁定表头2=========================
			var thead_ = $("#mydiv_" + instance).find("thead").html();
			var tds_ = $("#mydiv_" + instance).find("tr:first");
			//减去padding
			$("#mydiv1_" + instance).css({ "width": sfw[instance].width - 10 + "px" });
			$("#mydiv_" + instance).css({ "height": sfw[instance].height - 35 - 10 + "px", "width": sfw[instance].width - 10 + "px", "overflow": "auto" });

			$("#mydiv1_" + instance).find("thead").html(thead_);
			//alert($("#mydiv_"+instance).find("tbody").find("tr > td:eq(0) ").text());
			var td_len = "";
			var td_height = tds_.find("td:eq(0)").height();
			for (var i = 0; i < tds_.children().length - 1; i++) {
				td_len = tds_.find("td:eq(" + i + ")").width();
				//让数据的列宽与表头保持一致
				$("#mydiv1_" + instance).find("td:eq(" + i + ")").css({ "width": td_len - 6 + "px", "height": td_height });
				$("#mydiv_" + instance).find("tbody").find("tr > td:eq(" + i + ")").css({ "width": td_len - 6 + "px", "height": td_height });
			}
			$("#mydiv_" + instance).find("thead").html("");
   ...
   * */
		function initializeSuggestFramework(instance1, what, dtoname, width, height, columns, callBackFun, selectColumn, oneRowVisible) {
			...
			function addHandler(element, type, handler) {
				//给页面元素添加事件的通用方法，不理解的可以死记硬背下。写法是固定的。
				if (element.addEventListener) {
					// !IE//非IE下用addEventListener给元素添加事件
					element.addEventListener(type, handler, false);
				} else if (element.attachEvent) {
					// IE//IE下要用 attachEvent给元素添加事件
					element.attachEvent('on' + type, handler);
				} else {
					//DOM0级//都不支持的时候 就用这种方法 给元素添加事件
   ...
   */
		function suggestQuery(instance, url, param, paramIds) {
			sfwThrottle(instance);
			...
			var outstring = "";
			Base.submit(paramIds, url, param, null, false, function (data) {
   ...
   */
		function sfwQueryUtil(instance, method, param, isCallBackFn) {
			suggestQuery(instance, myPath() + method, { "dto['jstj']": param }, isCallBackFn);
		}
   ...
   */
		function sfwQueryUtilDY(instance, method, param, isCallBackFn, aae140) {
			suggestQuery(instance, myPath() + method, { "dto['jstj']": param, "dto['aae140']": aae140 }, isCallBackFn);
		}
   ...
   */
		function sfwQueryUtil(instance, method, param) {
			suggestQuery(instance, myPath() + method, { "dto['jstj']": param });
		}
		function sfwQueryUtil_newRPC(instance, method, param, paramIds) {
			suggestQuery(instance, myPath() + method, param, paramIds);
		}
   ...
   */
		function sfwQueryUtilForYl(instance, method, param, isCallBackFn) {
			suggestQuery(instance, myPath() + method, param, isCallBackFn);
		}
		/**
   *rapName 报表名   xxx.raq
   ...
   */
		function printRaq(rapName, param, isTrue) {
			...
			if (isTrue) {
				...
				Base.openWindow("dayin", Base.I18n.getLangText('taface.general.print'), toUrl, {}, "80%", "80%", null, null, true);
			} else {
				for (var i = 0; i < param.length; i++) {
					param_str = param_str + param[i];
					if (i != param.length - 1) {
				...
				$("#report1_printIFrame").attr("src", myPath() + "reportServlet?action=2&name=report1&reportFileName= " + rapName + "&" + "srcType=file&savePrintSetup=yes&appletJarName=runqian/runqianReport4Applet.jar%2Crunqian/dmGraphApplet.jar&" + "serverPagedPrint=no&mirror=no&paramString=" + param_str);
			}
		}
   ...
   */
		function sfwQueryUtilForMedicare(instance, method, param, isCallBackFn, localData) {
			...
			if (localData != null) {
				...
				suggestQueryForMedicare(instance, myPath() + method, { "dto['jstj']": param }, isCallBackFn, thrData);
			}
		}
   ...
   */
		...
		$.extend(this, {
			...
			"version": '3.12.0',
			"suggest": suggest,
			"sfwHighlight": sfwHighlight,
			"sfwSelectThis": sfwSelectThis
		});
	}
	return TaSuggest;
});
