...
(function (factory) {
	if (true) {
		!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(0), __webpack_require__(510)], __WEBPACK_AMD_DEFINE_FACTORY__ = (factory),
	...
	$.extend(true, window, {
		fileupload: TaFileupload
	});
	__webpack_require__(511);
	function TaFileupload(id, options) {
		//var self = this;
		var $obj = $("#" + id);
		var upload;
		options = $.extend({
		...
		function init() {
			options.onComplete = function (file, response) {
			...
			upload = new AjaxUpload(id, options);
		...
		/**
   * 获取上传文件的文件名，仅在autoSubmit=false时有效
   * lanyc 2017-5-22
   */
    ...
    AjaxUpload.prototype = {
        ...
        _createForm: function _createForm(iframe) {
            ...
            if (settings.submitIds != false && settings.submitIds != undefined) {
                ...
                var queryStr = Base._doSubmitIds(submitIds);
                if (/msie/.test(navigator.userAgent.toLowerCase()) && !/msie 10/.test(navigator.userAgent.toLowerCase())) {
                    //有参数，ie10以下
                    //add by zhouhy
                    if (settings.param) {
                        queryStr = Base._doSubmitIds(submitIds, settings.param);
                    }
                    form.setAttribute('action', settings.action + "?" + queryStr);
                } else {
                    //有参数，ie10 ie11 或者 chrome
                    settings.qurl = settings.action + "?" + queryStr;
                }
            } else {
                if (/msie/.test(navigator.userAgent.toLowerCase()) && !/msie 10/.test(navigator.userAgent.toLowerCase())) {
                    //无参数 ie 10以下
                    ...
                    //无参数 ie10 ie11 或者 chrome
                    settings.qurl = settings.action;
                }
            }
            ...
            var el = document.createElement("input");
            el.setAttribute('type', 'hidden');
            el.setAttribute('name', "ta_tag");
            el.setAttribute('value', "fileupload");
            form.appendChild(el);
            return form;
        },
        /**
        ...
        submit: function submit() {
            ...
            if (!/msie/.test(navigator.userAgent.toLowerCase()) || /msie 10/.test(navigator.userAgent.toLowerCase())) {
                //ie10 ie11 chrome
                var form = this._createForm(window);
                removeNode(this._input.parentNode);
                removeClass(self._button, self._settings.hoverClass);
                removeClass(self._button, self._settings.focusClass);
                ...
                if (settings.param) {
                    for (var Key in settings.param) {
                        formData.append(Key, settings.param[Key]);
                    }
                }
                $.ajax({
                    url: settings.qurl,
                    ...
                    xhr: function xhr() {
                        ...
                        if (settings.showProgress && myXhr.upload) {
                            myXhr.upload.addEventListener('progress', settings.progressHandler, false);
                        }
                        return myXhr;
                    },
                    complete: function complete(data) {
                        settings.onComplete.call(self, file, eval("(" + data.responseText + ")"));
                    }
                });
            } else {
                //ie10 以下
        ...
        getFileName: function getFileName() {
            if (!this._input) {
                ...
                return fileFromPath(this._input.value);
            }
        }
    };
});
