	...
	<body class="no-scrollbar no-padding">
		...
		<ta:box fit="true" layout="border" layoutCfg="{leftWidth:300}">
			<ta:box position="left" key="查询">
				<ta:box fit="true" heightDiff="40" id="userInfo">
					<ta:text id="managerType" display="false"/>
					<ta:text id="chief" display="false"/>
					<ta:text id="deputies" display="false"/>
					<ta:text id="loginid" key="登录号" labelWidth="60"/>
					<ta:text id="username" key="姓名" labelWidth="60"/>
					<ta:selectTree url="orgMgAction!webQueryAsyncOrgTree.do.do" labelWidth="60" idKey="orgid" nameKey="orgname" parentKey="porgid" asyncParam="['orgid']" fontCss="fnFontCss"
								   targetDESC="orgname" treeId="orgTree" targetId="orgid" key="部门" selectTreeBeforeClick="fnBeforeClick"/>
				</ta:box>
				<ta:buttonGroup align="center">
					<ta:button key="查询" onClick="fnClick()" icon="icon-search"/>
					<ta:button key="重置"  onClick="fnReset()" icon="icon-refresh"/>
				</ta:buttonGroup>
			</ta:box>
			<ta:box position="center" key="人员列表" cssStyle="padding:0px">
				<ta:panel fit="true"  withButtonBar="true" bodyStyle="padding:10px">
					...
					<ta:panelButtonBar align="center">
						<ta:button key="保存" icon="icon-addTo" onClick="fnSave()" />
						<ta:button key="关闭" icon="icon-close2" onClick="fnClose()"/>
					</ta:panelButtonBar>
				</ta:panel>
			</ta:box>
		</ta:box>
		...
		<%-- <ta:box height="10%">
			<ta:buttonLayout align="center">
				<ta:button key="保存" onClick="fnSave()"/>
