...
<head>
    ...
    <style>
        .label-style{float:left;height: 38px;line-height: 38px; font-size: 14px; color: #404040;}
    </style>
</head>
...
<ta:form id="form" cssStyle="padding:10px;">
    <ta:fieldset id="fset" cols="5">
        <div class="label-style">起止时间：</div>
        <ta:date id="begin" datetime="true" showSelectPanel="true" columnWidth="0.3"  bpopTipPosition="bottom"></ta:date>
        <div class="label-style">-</div>
        <ta:date id="end"  datetime="true" showSelectPanel="true" columnWidth="0.3"  bpopTipPosition="bottom"></ta:date>
        <ta:buttonGroup align="left">
            <ta:button type="submit" id="query" key="查询[Q]" hotKey="q" icon="icon-search" submitIds="form" url="serverExceptionLogController!query.do"/>
            <ta:button id="resetbt"  key="重置" icon="icon-refresh" onClick="Base.resetData('form');Base.clearGridData('MainGrid');Base.clearGridData('from');"/>
        </ta:buttonGroup>
    </ta:fieldset>
</ta:form>
<ta:panel id="MainPanel" key="主表信息"  fit="true" cssStyle="margin:0px 10px 10px 10px;">
    <ta:datagrid id="MainGrid" fit="true" selectType="checkbox" forceFitColumns="true" haveSn="true" columnFilter="true" dblClickEdit="true">
        ...
        <ta:dataGridToolPaging url="serverExceptionLogController!queryLinmit.do" submitIds="form"  pageSize="10"/>
    </ta:datagrid>
</ta:panel>
</body>
</html>
<script type="text/javascript">
    ...
    function fnClick(data,e){
        ...
        Base.openWindow('detailwin',"详细信息","serverExceptionLogController!goDetail.do",param,900,500,null,null,true);
    }
    function fnDelete(data,e){
        ...
        Base.confirm("确认删除?",function(yes){
                if(yes){
                    Base.submit('form',"serverExceptionLogController!delete.do",param,
                        function(){
                            return true;
                        },false,
                        function(data){
    ...
    /** 查询*/
    function fnQuery() {
        Base.submit("form,MainGrid", "serverExceptionLogController!query.do");
    }
