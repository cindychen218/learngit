...
<ta:form id="userForm" fit="true">
    <ta:panel id="userInfo" hasBorder="false" expanded="false" fit="true" bodyStyle="padding:10px 20px 10px 30px;" withButtonBar="true">
        <ta:fieldset key="人员基本信息" cols="2">
            ...
            <ta:text id="w_loginid" key="登录帐号" required="true" span="2"  validType='[{type:"maxLength",param:[10],msg:"最大长度为10"}]'/>
            <ta:text id="w_name" key="姓名" required="true"  validType='[{type:"maxLength",param:[10],msg:"最大长度为10"}]'/>
            ...
            <ta:text id="w_rpassword" key="确认口令" display="none" required="true" span="2" type="password" validType='[{type:"compare",param:["=","w_password"]}]'/>
            <ta:text id="w_yab003" key="分中心"/>
            <%--<ta:checkbox key="是否锁定" id="islock" value="islock"/>--%>
            <ta:text id="w_tel" key="移动电话"  validType='[{type:"maxLength",param:[11],msg:"最大长度为11"},{type:"mobile"}]'/>
        </ta:fieldset>
        <ta:panel key="所属组织" fit="true" withToolBar="true">
            <ta:panelToolBar cssStyle="height:auto;padding:0px">
                <ta:box cols="3">
                    ...
                    <ta:button asToolBarItem="true" key="添加" columnWidth="0.15" icon="icon-add2" onClick="fnAddOrgToOrgList()"/>
                    <ta:button asToolBarItem="true" key="删除" columnWidth="0.15" icon="icon-delete"  onClick="fnDelOrgFromOrgList()"/>
                </ta:box>
            </ta:panelToolBar>
        ...
        <ta:panelButtonBar>
            <ta:button type="submit" id="saveUserBtn" key="保存[S]" hotKey="S" icon="icon-addTo" submitIds="userInfo" url="userMgAction!webBatchOrgsUserSave.do" successCallBack="fnSaveSuccCb"/>
            <ta:button id="closeWinBtn" key="关闭[X]" hotKey="X" icon="icon-close2" onClick="Base.closeWindow('win');"/>
        </ta:panelButtonBar>
    </ta:panel>
</ta:form>
</body>
