...
<ta:pageloading/>
<ta:panel hasBorder="true" id="userPositionPanel"  withButtonBar="true" fit="true" bodyStyle="padding:10px;">
	<ta:box fit="true" cols="2">
		<ta:box id="form1" columnWidth="0.41" fit="true" cssClass="linecolor"
				cssStyle="padding:10px 0;background-color:white;">
			<ta:form id="clearForm" cols="2" fit="true">
				<ta:text id="open_userid" display="none"/>
				<ta:text id="positionids" display="none"/>
				<ta:selectTree columnWidth="0.75" url="positionUserMgAction!webQueryAsyncOrgTree.do"
							   selectTreeBeforeClick="fnBeforeClick" fontCss="fnFontCss" idKey="orgid"
							   parentKey="porgid" nameKey="orgname" asyncParam="['orgid']"
							   targetDESC="open_orgname" treeId="orgTreeUserPosition" targetId="open_orgid" key="组织"
							   labelWidth="70"/>
				...
				<ta:buttonGroup span="2" cssStyle="padding-top: 10px">
					<ta:button key="查询" hotKey="q" onClick="fnClick();" icon="icon-search"></ta:button>
					<ta:button key="重置" hotKey="r" onClick="Base.resetData('clearForm')" icon="icon-refresh"></ta:button>
				</ta:buttonGroup>
			</ta:form>
		</ta:box>
		<ta:box columnWidth="0.59" cssStyle="margin-left:10px;" cssClass="linecolor" fit="true">
			<ta:datagrid id="noPositionPerGrid" fit="true" haveSn="true" selectType="checkbox"
				...
				<ta:datagridItem id="positiontype" key="岗位类型" formatter="fnPositionType"
								 collection="POSITIONTYPE"></ta:datagridItem>
				<ta:datagridItem id="orgnamepath" key="岗位路径" showDetailed="true" width="250"></ta:datagridItem>
				<ta:dataGridToolPaging url="positionUserMgAction!getPubPositionsNoCurUseridByOrgId.do"
									   submitIds="form1" showExcel="false" pageSize="20"/>
			</ta:datagrid>
		</ta:box>
	</ta:box>
	<ta:panelButtonBar>
		<ta:button type="submit" icon="icon-addTo" submitIds="open_userid,noPositionPerGrid" url="positionUserMgAction!saveUserAddPositions.do"  key="保存[S]" hotKey="s" onSubmit="fnCheckSelectedData" successCallBack="fnAddPositionSuccess"/>
		<ta:button key="关闭[X]" icon="icon-close2" hotKey="x" onClick="parent.Base.closeWindow('win');"/>
	</ta:panelButtonBar>
</ta:panel>
</body>
</html>
<script>
    ...
    function fnAddPositionSuccess(){
        ...
        var param = {"userid":Base.getValue("open_userid")};
