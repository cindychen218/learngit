...
<ta:box fit="true">
    <ta:box cols="5" id="box_login">
        <ta:date id="taDate_login" key="查询日期" showSelectPanel="true"></ta:date>
        <ta:date id="taStartTime_login" key="开始时间" showSelectPanel="true" time="true"></ta:date>
        <ta:date id="taEndTime_login" key="结束时间" showSelectPanel="true" time="true" validNowTime="left"></ta:date>
        <ta:button key="查询" onClick="fnTaQuery_login()"></ta:button>
    </ta:box>
    <ta:box id="taLogAnalysis_login" cssStyle="height:300px;padding:0px 0px;margin:0px 30px;"></ta:box>

    <ta:panel key="人员登录信息" fit="true">
        <ta:datagrid id="grid_login" haveSn="true" snWidth="40" forceFitColumns="true" fit="true">
...
<script type="text/javascript">
	...
	function fnTaQuery_login() {
		//绘制用户时点在线统计图
		taLogAanalysisRun_login();
	}

	//获取当前在线信息
	function taGetLoginInfo() {
		var _id = '';
		var _url = '${basePath}security/loginLogAnalysisAction!getLoginInfo.do';
		var _param = {
			"dto['startTime']": Base.getValue('taStartTime_login'),
			"dto['endTime']": Base.getValue('taEndTime_login'),
			"dto['date']": Base.getValue('taDate_login')
		};
		Base.submit(_id, _url, _param);
	}


	function taStatLogin() {
		window.taLogAanalysisRun_login = run;

		//默认图样
		var defaultStyleType = 'line';
		var xInterval = 4;
		var time = {
			startTime: Base.getValue('taStartTime_login').split(':'),//['8','01'];
			endTime: Base.getValue('taEndTime_login').split(":")//['8','50'];
		}
		...
		function run(styleType) {
			...
			// srcipt标签式引入
			var myChart = echarts.init(document.getElementById('taLogAnalysis_login'));
			// 图表清空-------------------
			myChart.clear();
			...
			var data = Base.getJson("${basePath}security/loginLogAnalysisAction!analysisLoginNum.do", param);
