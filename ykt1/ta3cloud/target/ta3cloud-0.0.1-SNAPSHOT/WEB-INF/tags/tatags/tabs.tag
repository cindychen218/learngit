...
<%@attribute description='level1/level2 ,横向tabs的层级样式，默认level2,框架默认tabs样式.level1使用tabs一级样式' name='levelStyle' type='java.lang.String' %>
...
(function(factory){
	if (typeof require === 'function') {
		require(["jquery","TaUIManager","tabs"], factory);
	} else {
		factory(jQuery);
	}
}(function($){
	Ta.core.TaUICreater.addUI( function(){
	  var TaTabs = window.tabs;
	  var options = {
	      	id : "<%=id%>",
	   	};
	  
	  Ta.core.TaUIManager.register('<%=id%>',new TaTabs('<%=id%>',options));
	});
}));
</script>