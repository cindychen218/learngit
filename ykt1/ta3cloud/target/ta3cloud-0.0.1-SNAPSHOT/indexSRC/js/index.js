...
var indexCommon = (function () {
    ...
    /*初始化菜单显示*/
    function _initMenu(menuList,menuInitOne) {
        var menuLeve1OneStyle = getWebConfig("menuLeve1OneStyle"),
            menuLevelThreeStyle = getWebConfig("menuLevelThreeStyle");
        menuLeve1OneStyle == "horizon" ? menuLevelOneHorizon.init(menuList,menuInitOne) : menuLevelOneDropdown.init(menuList,menuInitOne);
    }
    ...
    function _initGuide(){
        ...
        $(".guide-btn").on({
            mouseenter :function(){
                $(".guide-content").removeClass("display")
            },
            mouseleave:function(){
                $(".guide-content").addClass("display")
            }
        })
    }
    return {
        init: function (menuList,menuInitOne) {
            _initStyle();
            _initMenu(menuList,menuInitOne);
            _initLeftPartContol();
            _initIndexTab();
            _initGuide();
        },
