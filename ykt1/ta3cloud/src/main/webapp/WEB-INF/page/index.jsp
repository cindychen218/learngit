...
<%
    ...
    String menuInitId = (String) request.getAttribute("menuInitId");
    String addTabId = (String) request.getAttribute("addTabId");
%>
...
<div class="top-part">
    <%--logo部分start--%>
    <div class="logo"></div>
    <div class="logo-title" style="width: <%=session.getAttribute("systemNameWidth")%>px">演示系统</div>
    <%--logo部分end--%>
    <%--顶部横向菜单start   --%>
    ...
    <div class="user-info">
        ...
        <span onclick="logout()" class="exit-btn">退出登录</span>
    </div>
    <%--<div class="user-info-control icon-person" onclick="showUserInfo()"></div>--%>
    <%--用户信息切入框 end  --%>
</div>
<%--顶部部分end--%>
...
<div class="center-part">
    ...
    <div class="right-part" id="right-part">
        <div class="index-tabs">
            <%--里面装tabs标签功能的--%>
            <div class="left-part-control faceIcon icon-retract active" id="left-part-control"></div>
            <div class="index-tab-gzt active" id="tab_01">概览</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        var addTabId = <%=addTabId%>||"";
        //初始化菜单
        indexCommon.init(menuList,'<%=menuInitId%>');
        //初始化用户信息
        UserInfoInit();

        //pc端首页点击进入
        if(addTabId != ""){
            getMenuObj(menuList,addTabId);
            if(addTabMap.menuId){
                indexTab.addTab(addTabMap.menuId, addTabMap.menuName, addTabMap.url, false);
            }
        }

    });

    function getMenuObj(menuList,id){
        for(var i = 0;i < menuList.length;i++){
            var temMap = menuList[i];
            var childNode = temMap.childNode||[];
            if(childNode.length != 0){
                getMenuObj(childNode,id);
            }else if(temMap.menuId == id){
                addTabMap = temMap;
            }
        }
    }
