...
<%@tag import="com.yinhai.core.common.api.base.IPage"%>
...
<%@attribute description='是否显示边框，默认true' name='border' type='java.lang.String' %>
...
<%@attribute description='是否可移动行' name='moveRows' type='java.lang.String' %>
<%@attribute description='是否显示表头搜索,与searchType联合使用' name='showHeaderSeach' type='java.lang.String' %>
<%@attribute description='冻结列使用方法 frozenColumn=4,冻结前4列' name='frozenColumn' type='java.lang.String' %>
<%@attribute description='是否显示表格控制列显示的按钮' name='showColumnControl' type='java.lang.String' %>
...
<%
	...
	if(resultBean != null){
		Map<String, IPage<?>> map = resultBean.getGridsPageData();
		if(map !=null){
			if(!map.isEmpty()){
				IPage pageBean = map.get(id);
				data = JSonFactory.bean2json(pageBean);
			}
		}
	}
	//查找request
	if((data == null || "".equals(data)) && request.getAttribute(id) != null){
	...
	//查找session
	if((data == null || !"".equals(data)) && request.getSession().getAttribute(id) != null){
	...
	if ((id == null || id.length() == 0)) {
		...
		nextInt = nextInt == Integer.MIN_VALUE ? Integer.MAX_VALUE : Math
				.abs(nextInt);
		id = "tadatagrid_" + String.valueOf(nextInt);
...
<% }%>
<% if( null != showColumnControl)  { %>
o.showColumnControl = ${showColumnControl};
<% }%>
...
<% if (null != forceFitColumns) {%>
	o.forceFitColumns = ${forceFitColumns};//change by zzb
<% }%>
...
<% }%>
<% if (null != moveRows) {%>
	o.moveRows = ${moveRows};//add by zzb
<% }%>
<% if (null != showHeaderSeach) {%>
o.showHeaderSeach = ${showHeaderSeach};//add by cy
<% }%>
var data = ${data};
