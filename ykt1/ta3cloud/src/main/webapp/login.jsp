...
<head>
    ...
    <%--<link href="${pageContext.servletContext.contextPath }/loginSRC/css/master.css" rel="stylesheet" type="text/css" />--%>
    <link href="${pageContext.servletContext.contextPath }/loginSRC/css/login.css" rel="stylesheet" type="text/css" />
    <%--登陆页面样式覆盖--%>
...
<div style="height: 100%;width: 100%;">
    <img src="loginSRC/img/cas/BG1.png" class="bg-all"/>
    <img src="loginSRC/img/cas/bg-top.png" class="bg-top"/>
    <div class="login-top">
        <div class="login-logo clearfloat" style="width: 824px">
            <img src="loginSRC/img/cas/logo.png" style="height: 50%;width: 50%" />
        </div>
    </div>
    <div class="login-center" id="loginCenter" style="height: 433.55px;width: 824px;">
        <div class="login-inputs">
            <form onsubmit="return _login();" role="form" method="post" id="fm1" name="loginform" modelAttribute="credentials"  commandName="${commandName}" htmlEscape="true">
                <div class="form-welcome">
                    <b>欢&nbsp;迎&nbsp;登&nbsp;录</b>
                </div>
                <div class="form-item item-first">
                    <div class="input-img person"></div>
                    <span>用户名</span>
                    <%--<input id="username"  tabindex="1" class="input" type="text" autocomplete="false" name="username" value="${sessionScope.username}"/>--%>
                    <%--<input id="username_display"  tabindex="1" class="input" type="text"  size="25" autocomplete="false" style="display:none;"/>--%>
                    <input type="text" id="j_username"
                           onkeyup="usernameKeyup(this)" name="j_username" class="input" border="0"  />
                </div>
                <div class="form-item">
                    <div class="input-img lock"></div>
                    <span>密码</span>
                    <%--<input type="password" class="input" cssErrorClass="form-control" id="password" size="25" tabindex="2" path="password" htmlEscape="true" autocomplete="off" />--%>
                    <input type="password" id="j_password" autocomplate="off"
                           onkeyup="passwordKeyup(this)" name="j_password" class="input" border="0"/>
                </div>
                <div class="form-item form-item-verification" id="checkCodeDiv">
                    <div class="input-img verification"></div>
                    <span>验证码</span>
                    <img id="codeimg" class="vcodeimg" onclick="javascript:refeshCode();" title="点击刷新验证码" height="32" width="70" src="<%=request.getContextPath() %>/CaptchaImg" style="cursor: pointer;"/>
                    <%--<input class="required form-control vcode" id="vcode" size="25" tabindex="3" path="vcode" htmlEscape="true" autocomplete="off" />--%>
                    <input type="text" id="checkCode"
                           onkeyup="checkCodeKeyup(this)" name="checkCode" class="required form-control vcode" border="0"/>
                </div>

                <div class="form-tip clearfloat">
                    <a class="btn-chgpwd" href="javascript:void(0)" onclick="fnOpenModifyPsd()">修改密码</a>
                </div>
                <div class="error-msg">
                    <div class="allerrors">
                        <div id="validate" class="validate">${sessionScope['SPRING_SECURITY_LAST_EXCEPTION'].message} </div>
                    </div>
                </div>
                <div class="btn_login clearfloat">
                    <%--<a class="btn-submit" id="doLogin" onclick="login()">立即登录</a>--%>
                    <input type="button" id="submit"
                           class="btn-submit" border="0" value="登&nbsp;录" onclick="login()"/>
                </div>
                ...
                <input type="hidden" name="caType" />
                <input type="hidden" name="qrCode" />
                <input type="hidden" name="additionsString" value="{'csrfToken':'<%=encryptCsrfToken%>'}" />--%>
            </form>
            <div class="login-tools">
                <a href="javascript:window.external.AddFavorite(window.location.href,document.title);" class="tool-icon-01"><span>加入收藏</span></a>
                <a href="javascript:void(0)" class="tool-icon-02" style="margin-left: 49.5px"><span>设为主页</span></a>
                <a href="javascript:fnOpenDownloadPage();" class="tool-icon-03" ><span>资料下载</span></a>
            </div>
        </div>
        <img src="loginSRC/img/cas/bottom.png" class="bg-center-bottom"/>
    </div>
</div>
<%--<%@include file="/html/downloadList.html" %>--%>
...
<!--[if !IE]><!-->
<div id="winBack" class="win-back"></div>
<!--<![endif]-->
<!--[if IE]>
<div id="winBack" class="win-back-ie"
     style=" filter: progid:DXImageTransform.Microsoft.Alpha(opacity=50)"></div>
<![endif]-->
<%-- <div class="login-bottom">
    <spring:message code="bottom.hintinfo" />
</div> --%>
</body>
...
<script type="text/javascript">
    ...
    window.onload = function(){//等待页面内DOM、图片资源加载完毕后触发执行
        var clientHeight = cHeight = document.documentElement.clientHeight;
        $(".login-center").height(clientHeight*0.575);
        $(".login-img").height(clientHeight*0.0791);
        $(".login-top").css("margin-top",(clientHeight-$(".login-top").height()-$(".login-center").height()-33)/2+"px");
        $(".login-center").width($(".login-center").height()*1.6);
        $(".login-logo").width($(".login-center").height()*1.6);
        $(".vcode").width($(".input").width()-$(".vcodeimg").width());
        $(".tool-icon-02").css("margin-left",$(".tool-icon-02").parent().width()/2-85+"px");
