...
var menuLevelOneHorizon = (function () {
    ...
    function _createMenuLevelOneHorizon() {
        ...
        if (menuList.length > 0) {
            ...
            active_item ? $("#" + active_item).addClass("active") : $("#" + menuList[curMenuIndex].menuId).addClass("active");
            $(".menu-one-dropdown-control").html( menuList[curMenuIndex].menuName||"")
        }
    }
    ...
    var curMenuIndex = 0;
    return {
        init: function (menulist,menuInitOne) {
            menuList = menulist;
            if(menuList){
                var initIndex = curMenuIndex;
                if(typeof menuInitOne != 'undefined'){
                    initIndex = menuList.findIndex(function (menu) {
                        return menu.menuId == menuInitOne
                    })
                    if(initIndex == -1) initIndex = 0;
                }
                curMenuIndex = initIndex;
            }

            _createMenuLevelOneHorizon();
            _bindMenuItemEvent();
            if(menuList){
                menuList[curMenuIndex]?( menuList[curMenuIndex].childNode? menuLevelTwo.init(menuList[curMenuIndex].childNode):menuLevelTwo.init([])):menuLevelTwo.init([]);
            }
...
var menuLevelOneDropdown = (function () {
    var menuList;
    var $menucon = $("#menu-one-dropdo wn");
    ...
    function _setControl() {
        ...
        if(menuList && menuList.length){
           ...
           if(active_item){
               ...
               $("#" + menuList[curMenuIndex].menuId).addClass("active");
               $menucon.attr("data-active",menuList[curMenuIndex].menuId);
               $control.html(menuList[curMenuIndex].menuName);
               _nextMenuInit(curMenuIndex);
           }
        }
    }
    function _bindMenuItemEvent() {
