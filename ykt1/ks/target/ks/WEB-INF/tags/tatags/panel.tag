...
<%@attribute name="key" type="java.lang.String" description="组件头的标题"%>
...
<%@attribute name="titleAlign" type="java.lang.String" description='left/center/right,panel标题的位置，默认left'%>
...
<%@attribute name="minHeight" type="java.lang.String" description='设置最小高度'%>
<%-- @doc --%>
<%
...
<%		
		 ...
		 */
		String tcssStyle = "",wcssStyle = "";
		if (width != null) {
			String tmpw = width;
            wcssStyle = "width:"+(tmpw.endsWith("px")?tmpw:(tmpw+"px"))+";";
            if(cssStyle!=null){
				cssStyle = wcssStyle +cssStyle;
			}else{
				cssStyle = wcssStyle;
			}
		}
...
> 
	<%if(!ValidateUtil.isEmpty(key)) {%>
		...
		<%} %>
			<div class="panel-title <%if(titleAlign != null){%>${titleAlign}<%}else{%>left<%}%>" title="${key}">
				 ${key} 	
			</div>
			<div class="panel-tool-box">
				...
				<div class="header-button">
					...
					%>
					<button id="<%=obj.get("id") %>" class="panel_header_button tabutton linkbutton small btn-main" onclick="<%=obj.get("click")%>"  type="button">
						<span class="button-text"><%=obj.get("name") %></span>
					</button>
					<%
						}
					%>
				...
				<div class="panel-tool">
					...
					<div class="panel-tool-max faceIcon icon-maximization"  title="放大或缩小"></div>
					<% }%>
					<%if("false".equals(expanded)){} else{%>
					<div class="panel-tool-collapse faceIcon icon-dbArrow_down" title="收缩或展开"></div>
					<% }%>
				</div>
				<%} %>
			</div>
...
<script>
	(function(factory){
		if (typeof require === 'function') {
			require(["jquery","TaUIManager","tauipanel"],factory);
		}else{
			factory(jQuery);
		}
	}(function($){
		Ta.core.TaUICreater.addUI(
				function(){$("#<%=id%>").tauipanel();}
