...
(function () {
    var $ = {
        ...
        $skinList: [
            {name: "freshBlue", charset: "UTF-8"},
            {name: "deepBlue", charset: "UTF-8"},
            {name: "shebaoBlue", charset: "UTF-8"}
        ],
        $wdate: true,
        $crossFrame: true,
        $preLoad: false,
    ...
    //add by xp 添加自定义控制
    function defineContorl($dp,G){
        //自定义控制
        if(G.defineConfig){
            if(!$dp || !$dp.dd){
                var begin = "<script type=\"text/javascript\">";
                var end = "</script>"
                var defineScript = "";

                //控制操作按钮显示内容
                if(G.defineConfig.$lang){
                    for(var i in G.defineConfig.$lang[G.lang]){
                        defineScript += "$lang[\""+i+"\"]=\""+G.defineConfig.$lang[G.lang][i]+"\"";
                    }
                }
                if(defineScript != ""){
                    defineScript = begin + defineScript + end;
                }
                return defineScript;
            }else if ($dp.cal){
                //控制操作按钮显示内容
                if(G.defineConfig.$lang){
                    for(var i in G.defineConfig.$lang[G.lang]){
                        Base.sendMsgToFrame("function","(function(value){document.getElementById('my97datedd').contentWindow.document.getElementById('dpTodayInput').value = value;})",G.defineConfig.$lang[G.lang][i]);
                    }
                }
            }
    ...
    function I(G, _) {
        ...
        function E(K, J) {
            var I = V[N].domain, F = false, G = "<iframe style='box-shadow:0px 0px 10px rgba(140, 147, 152, .45);background-color: rgba(140, 147, 152, .45);' id=my97datedd hideFocus=true width=9 height=7 frameborder=0 border=0 scrolling=no src=\"about:blank\"></iframe>";
            K.innerHTML = G;
            ...
            function C() {
                ...
                for (var C = 0; C < D.length; C++)if (D[C].name == J.skin)$.push("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + A + "css/" + D[C].name + ".css\" charset=\"" + D[C].charset + "\"/>");
                $.push("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + A + "fonts/WdateIcon.css\" />");
                $.push("<script src=\"" + A + "calendar.js\"></script>");

                //自定义控制
                var defineScript = defineContorl(null,J);
                if(defineScript){
                    $.push(defineScript);
                }
        ...
        function B(J) {
            var H = J.position.left+9, C = J.position.top, D = J.el;
            if (D == T)return;
            ...
            //add by cy解决ie9下年月时间期号显示为1的情况
            var userAgent = navigator.userAgent;
            var reIE = new RegExp("MSIE (\\d+\\.\\d+);");
            reIE.test(userAgent);
            var fIEVersion = parseFloat(RegExp['$1']);
            if (fIEVersion == 9 ){
                if(this.dateFmt && (this.dateFmt=="yyyy" || this.dateFmt=="yyyy-MM" ||  this.dateFmt=="yyyyMM" || this.dateFmt=="HH:mm")){
                    try {
                        var ifr = J.dd.childNodes[0].contentWindow.document.
                ...
                }else if(this.$dp &&　this.$dp.dateFmt && (this.$dp.dateFmt =="yyyy" || this.$dp.dateFmt =="yyyy-MM" ||  this.$dp.dateFmt =="yyyyMM"||this.$dp.dateFmt=="HH:mm" )){
                    try {
                        var ifr = J.dd.childNodes[0].contentWindow.document.
                            childNodes[0].childNodes[1].childNodes[1].childNodes[1];
                        if (ifr) {
